...
drives = [r'C:\Onedrives\OneDrive - abc\Séries', r'D:\OneDrives\OneDrive - abc\Filmes\Series'] #PC Casa
pastas = os.listdir(drives[i])
for pasta in pastas:
#    x = re.search('^[A-Z].*[a-z]$', pasta)
# pesquisa no nome da pasta
...
for pasta_a_corrigir in pastas_a_corrigir:
    for pasta_base in pastas_base:
        if pasta_base in pasta_a_corrigir or pasta_base in pasta_a_corrigir.replace('.', ' '):
            caminho_completo = os.path.join(drives[i], pasta_a_corrigir)
            caminho_completo_novo = os.path.join(drives[i], pasta_base)
